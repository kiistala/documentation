...
Durante l'installazione del {opsi-server}, dovrete autorizzare il patching dei files `dhcpd.conf` and `smb.conf`. Rispondete 'yes' alle domande. Vi sarà anche richiesta la password dell'utente 'pcpatch'. Impostate una nuova password, e ricordate questa password nel prosieguo delle seguenti sezioni del manuale.

Durante l'installazione del {opsi-server}, ignorate ogni avvertimento circa la mancanza del file `/etc/opsi/modules`.

Se volete eseguire l'interfaccia di gestione '{opsi-configed}' direttamente sul server, dovrete installare Java Runtime Environment di Sun/Oracle. Per installare questi pacchetti, eseguite questi comandi:
...
==== Verifica della configurazione di JavaThis will work fine if you start

Il {opsi-server}s ed i clienti connessi, sono amministrati con il programma '{opsi-configed}'. Il programma è scritto in Java, e richiede la versione 6 di Java minimo (o la versione 1.6 nella vecchia nomenclatura).
...
==== Uso del server DHCP fornito con la VM {opsi-server}
...
Se volete usare un server DHCP esterno, dovrete disabilitare il server DHCP del {opsi-server} con l'uso dei seguenti comandi :
...
* Attraverso un click destro del mouse sul desktop dell'interfaccia del server aprirete un menu contestuale, dove potrete scegliere „opsi config editor“.
